...
def choose(paragraphs, select, k):
    ...
    if k >= 0 and k < len(valid_paragraphs):
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def choose(paragraphys):
        clear = remove_punctuation(paragraphys)
        lower_word = lower(clear)
        words = split(lower_word)
        for word in words:
            if word in topic:
                return True
        return False
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for valid_word in valid_words:
        if valid_word == typed_word:
            return valid_word
    limits = min([[diff_function(typed_word, word, limit), word] for word in valid_words], key = lambda item: item[0])
    if limits[0] <= limit:
        return limits[1]
    else:
        return typed_word
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def feline_flips_helper(start, goal, count):
        if count > limit:
            return float('inf')
            ...
            if start[0] != goal[0]:
                count += 1
            return feline_flips_helper(start[1:], goal[1:], count)
    return feline_flips_helper(start, goal, abs(len(start) - len(goal)))
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == goal: # Fill in the condition
        ...
        # BEGIN
        "*** YOUR CODE HERE ***"
        return len(goal) if len(goal) <= limit else float('inf')
        # END
    elif len(goal) == 0:
        return len(start) if len(start) <= limit else float('inf')
    elif limit < 0:
        return float('inf')
    else:
        ...
        "*** YOUR CODE HERE ***"
        if goal[0] == start[0]:
            same = minimum_mewtations(start[1:], goal[1:], limit)
            return min(same, add, remove, substitute)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    time = get_times(match)
    fastest_word = [[] for _ in player_indices]
    for j in word_indices:
        index = 0
        for i in player_indices:
            if time[i][j] < time[index][j]:
                index = i
        fastest_word[index].append(word_at(match, j))
    return fastest_word
    # END PROBLEM 10
